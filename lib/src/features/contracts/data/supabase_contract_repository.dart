...
class SupabaseContractRepository implements ContractRepository {
  ...
  Future<List<Contract>> getContracts({
    ...
    var query = _client.from('financing_contracts').select(
          '*, customers(full_name), inventory_items(make, model, license_plate)',
        );
  ...
  Future<Map<String, dynamic>> getContractStats() async {
    // Senior Fix: استخدام head: true مع CountOption لجلب العدد فقط دون البيانات لتحسين الأداء
    final responses = await Future.wait([
      _client.from('financing_contracts').select('*'),
      _client.from('financing_contracts').select('*').eq('status', 'active'),
      _client.from('financing_contracts').select('*').eq('status', 'draft'),
      _client.from('financing_contracts').select('*').eq('status', 'defaulted'),
    ]);
