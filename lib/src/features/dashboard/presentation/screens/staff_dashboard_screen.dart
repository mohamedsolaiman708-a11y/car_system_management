...
class StaffDashboardScreen extends ConsumerWidget {
  ...
  Widget _buildWelcomeHeader(String name) {
    return Column(
      ...
      children: [
        ...
        Text(
            'مرحباً بك مجدداً، $name. إليك ملخص العمليات اليوم.',
            style:
  ...
  Widget _buildTopStatsGrid(
      Map<String, dynamic> stats, intl.NumberFormat f) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return GridView.count(
          ...
          mainAxisSpacing: 20,
          children: [
            _StatCard('إجمالي العملاء',
                (stats['total_customers'] ?? 0).toString(),
                Icons.people_alt_rounded, Colors.indigo),
            _StatCard('السيارات المتاحة',
                (stats['available_cars'] ?? 0).toString(),
                Icons.directions_car_filled_rounded, Colors.yellow),
            _StatCard('العقود النشطة',
                (stats['active_contracts'] ?? 0).toString(),
                Icons.assignment_turned_in_rounded, Colors.orange),
            _StatCard('الأقساط المستحقة',
                f.format(stats['total_due_installments'] ?? 0),
                Icons.payments_rounded, Colors.blue),
            _StatCard('أرصدة المستثمرين',
                f.format(stats['investor_balances'] ?? 0),
                Icons.account_balance_wallet_rounded, AppColors.accentGold),
            _StatCard('سيولة الصندوق',
                f.format(stats['cash_liquidity'] ?? 0),
  ...
  Widget _buildMainSection(
      Map<String, dynamic> stats, intl.NumberFormat f, BuildContext context) {
      ...
      children: [
            ...
            child: Center(
              ...
              children: [
                Icon(Icons.bar_chart_rounded,
                    size: 80, color: AppColors.bgGrey),
                SizedBox(height: 16),
                Text('الرسم البياني سيكون متاحاً في المرحلة 18',
        ...
        Expanded(
          ...
          child: _AppCard(
            ...
            child: Column(
              children: [
                _OverdueItem('أكثر من 90 يوم',
                    f.format(stats['overdue_over_90'] ?? 0),
                    Colors.red,
                    stats['c_max'] ?? 0),
                _OverdueItem('من 60 إلى 90 يوم',
                    f.format(stats['overdue_60_90'] ?? 0),
                    Colors.orange,
                    stats['c_90'] ?? 0),
                _OverdueItem('من 30 إلى 60 يوم',
                    f.format(stats['overdue_30_60'] ?? 0),
                    Colors.amber,
                    stats['c_60'] ?? 0),
                _OverdueItem('أقل من 30 يوم',
  ...
  Widget _buildRecentAndActions(
      Map<String, dynamic> stats, BuildContext context) {
    bool isCompact = MediaQuery.of(context).size.width < 1100;
    final recentContracts =
        (stats['recent_contracts'] as List?) ?? [];
              ...
              children: [
                if (recentContracts.isEmpty)
                  const Padding(
                      ...
                      child: Text('لا توجد عقود مسجلة حالياً'))
                else
                  ...recentContracts.map((c) => ListTile(
                        leading: const CircleAvatar(
                            ...
                            child: Icon(Icons.description_outlined,
                                size: 18)),
                        title: Text(c['contract_no'] ?? '-',
                            style: const TextStyle(
                                fontWeight: FontWeight.bold)),
                        subtitle: Text(
                            c['customers']?['full_name'] ?? '-'),
                        trailing: Text(
                            '${c['total_contract_value'] ?? 0} ر.س',
                            style: const TextStyle(
                                color: AppColors.accentGold,
                                fontWeight: FontWeight.bold)),
                        onTap: () =>
                            context.push('/contracts/${c['id']}'),
                      )),
              ],
            ),
          ),
            ...
            child: Wrap(
              ...
              children: [
                _QuickAction(
                    ...
                    color: Colors.blue,
                    onTap: () => context.push('/contracts/new')),
                _QuickAction(
                    label: 'عميل جديد',
                    icon: Icons.person_add_rounded,
                    color: Colors.green,
                    onTap: () => context.push('/crm/new')),
                _QuickAction(
                    label: 'سيارة جديدة',
...
class _AppCard extends StatelessWidget {
  ...
  final double? height;
  const _AppCard(
      {required this.title, required this.child, this.height});
