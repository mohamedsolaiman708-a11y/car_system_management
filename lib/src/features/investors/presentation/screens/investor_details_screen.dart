  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return investorAsync.when(
      ...
      error: (err, _) => Scaffold(
        body: Center(child: Text(Failure.fromException(err).message)),
      ),
    );
  }
}
...
class _FinancialTab extends ConsumerWidget {
        ...
        child: Column(
          children: [
            ...
            _SectionHeader(
              ...
              subtitle: 'تفاصيل الإيداعات والأرباح والسحوبات',
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          return ListTile(
                            leading: CircleAvatar(
                              radius: 16,
                              backgroundColor: (isPlus ? _green : _red)
                                  .withOpacity(0.1),
                              child: Icon(
                                isPlus
  ...
  void _showTxDialog(
    ...
    showDialog(
      context: ctx,
      builder: (_) => AddTransactionDialog(investorId: invId, type: type),
    );
  }
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return contractsAsync.when(
      data: (contracts) => contracts.isEmpty
          ? _emptyState('لا توجد عقود ممولة')
          : ListView.builder(
              padding: const EdgeInsets.all(24),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'مساهمة المستثمر: ${f.format(item['amount_allocated'])} ر.س',
                              style: const TextStyle(
                                color: Colors.grey,
                                fontSize: 11,
                              ),
...
class _WithdrawalRequestsTab extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_WithdrawalRequestsTab> createState() =>
      _WithdrawalRequestsTabState();
}

class _WithdrawalRequestsTabState
    extends ConsumerState<_WithdrawalRequestsTab> {
  String? _processingId;
  @override
  Widget build(BuildContext context) {
    final requestsAsync = ref.watch(
      withdrawalRequestsControllerProvider(investorId: widget.investorId),
    );
    final f = intl.NumberFormat.currency(symbol: '', decimalDigits: 2);
    return requestsAsync.when(
      data: (reqs) => reqs.isEmpty
          ? _emptyState('لا توجد طلبات سحب')
          : ListView.builder(
              padding: const EdgeInsets.all(24),
                      ...
                      if (isPending)
                        TextButton(
                          onPressed: _processingId != null
                              ? null
                              : () => _approve(r['id']),
                          child: const Text('اعتماد التنفيذ'),
                        )
                      else
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return projectionsAsync.when(
      data: (list) => list.isEmpty
          ? _emptyState('لا توجد تدفقات متوقعة')
          : ListView.builder(
              padding: const EdgeInsets.all(24),
...
Widget _statusBadge(String s) {
  ...
  return Container(
    ...
    child: Text(
      s == 'active' ? 'نشط' : (s == 'completed' ? 'تم' : 'قيد الانتظار'),
