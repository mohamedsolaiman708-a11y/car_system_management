...
class _CustomersScreenState extends ConsumerState<CustomersScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.transparent, // لأن الخلفية تأتي من الـ Scaffold الرئيسي
      body: Padding(
    ...
    return Row(
      ...
      children: [
        const Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('قاعدة بيانات العملاء', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.primaryNavy)),
            Text('إدارة وتتبع بيانات العملاء والمخاطر الائتمانية', style: TextStyle(color: AppColors.textGrey, fontSize: 13)),
          ],
        ),
      ...
      child: ClipRRect(
        ...
        child: DataTable(
          ...
          columns: const [
            ...
            DataColumn(label: Text('رقم الهوية', style: TextStyle(fontWeight: FontWeight.bold))),
            DataColumn(label: Text('رقم الجوال', style: TextStyle(fontWeight: FontWeight.bold))),
            DataColumn(label: Text('درجة المخاطر', style: TextStyle(fontWeight: FontWeight.bold))),
          ...
          rows: customers.map((c) => DataRow(
            cells: [
              DataCell(Row(
                children: [
                  CircleAvatar(radius: 18, backgroundColor: AppColors.primaryNavy.withOpacity(0.1), child: Text(c.fullName[0], style: const TextStyle(fontSize: 12, color: AppColors.primaryNavy))),
                  const SizedBox(width: 12),
              ...
              DataCell(IconButton(icon: const Icon(Icons.arrow_forward_ios_rounded, size: 14), onPressed: () => context.push('/crm/customers/${c.id}'))),
            ],
          )).toList(),
        ),
      ),
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            leading: CircleAvatar(backgroundColor: AppColors.bgGrey, child: Text(c.fullName[0])),
  ...
  Widget _buildActionButton(IconData icon, String label) {
    return Container(
      ...
      child: IconButton(icon: Icon(icon, size: 20, color: AppColors.primaryNavy), onPressed: () {}),
    );
  }
